...
int tegra_cpu_scale_sclk_rate(unsigned long cpu_rate)
{
	static struct clk *cpu_sclk = NULL;
	...
	/* Vote on SCLK frequency based on cpu frequency;
	   input cpu rate is in kHz, sclk rate is in Hz */
	cpu_rate *= 1000;
	if (cpu_rate > tegra_clk_virtual_cpu_g.min_rate * 2)
		sclk_rate = 80000000;
	else if (cpu_rate > tegra_clk_virtual_cpu_g.min_rate)
		sclk_rate = 40000000;
	else
		sclk_rate = 0;		/* sclk min */

	return clk_set_rate(cpu_sclk, sclk_rate);
}
#endif
