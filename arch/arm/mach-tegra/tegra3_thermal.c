...
static void tegra_therm_throttle(bool enable)
{
	if (throttle_enb != enable) {
		//mutex_lock(&thermal_state.mutex); // NVBUG 933337, Duplicate lock. Disable
		pr_err("%s: tegra_throttling_enable(%d)", __func__, enable);
		tegra_throttling_enable(enable);
		throttle_enb = enable;
		//mutex_unlock(&thermal_state.mutex); // NVBUG 933337, Duplicate lock. Disable
	}
}
#endif
...
#undef EDP_TEMP_TJ
	...
	pr_err("%s: lo/temp/hi[%d/%d/%d]\n", __func__, tj2dev(thermal->device, lo_limit_tj),
	    temp_dev, tj2dev(thermal->device, hi_limit_tj));
