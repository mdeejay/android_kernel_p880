...
static struct tegra_usb_phy *phyClone;
...
static irqreturn_t usb_phy_vbus_irq_thr(int irq, void *pdata)
{
	...
	if (system_state == SYSTEM_BOOTING)
		return IRQ_HANDLED;
       /*
        * Be sure to keep below USB power sequence to prevent damage to AP.
        * Power-Up : AVDD_USB on -> USB_VBUS on
        * Power-Down : USB_VBUS off -> AVDD_USB Off
        */
	if (irq == usb_phy_data[phy->instance].vbus_irq) {
		gpio_set_value_cansleep(usb_phy_data[phy->instance].vbus_gpio, 0);
		return IRQ_HANDLED;
	}
	...
	if (irq == usb_phy_data[phy->instance].vbus_irq + 1)
		gpio_set_value_cansleep(usb_phy_data[phy->instance].vbus_gpio, 1);
...
void init_booting_USB_vbus(void)
{
	if (max77663_get_acok_stat())
	{
		printk("\nVbus = up\n");
		if (phyClone->reg_vdd && !phyClone->regulator_on) 
		{
			regulator_enable(phyClone->reg_vdd);
				phyClone->regulator_on = 1;
			/*
			 * Optimal time to get the regulator turned on
			 * before detecting vbus interrupt.
			 */
		...
		gpio_direction_output(usb_phy_data[0].vbus_gpio, 1);
		tegra_otg_check_vbus_detection();
	}
}

EXPORT_SYMBOL(init_booting_USB_vbus);
			...
			enum tegra_usb_phy_type usb_phy_type)
{
	...
	if (phy->usb_phy_type == TEGRA_USB_PHY_TYPE_UTMIP) {
		...
		uhsic_config = config;
		if(uhsic_config->enable_gpio > 0 && uhsic_config->reset_gpio > 0)
		{
	...
	if (instance == 0 && usb_phy_data[0].vbus_irq) {
		err = 0;
		gpio_request(usb_phy_data[0].vbus_gpio,"usb_phy_vbus_ctl");
		err |= gpio_direction_output(usb_phy_data[0].vbus_gpio, 0);
		err |= request_threaded_irq(usb_phy_data[0].vbus_irq, NULL,
			usb_phy_vbus_irq_thr, IRQF_SHARED, "usb_phy_vbus_f", phy);
		err |= request_threaded_irq(usb_phy_data[0].vbus_irq+1, NULL,
			usb_phy_vbus_irq_thr, IRQF_SHARED, "usb_phy_vbus_r", phy);
...
int tegra_usb_phy_bus_connect(struct tegra_usb_phy *phy)
{
	...
	struct tegra_uhsic_config *ulpi_config=phy->config;
	static int b_first=0;
#endif	
	struct tegra_uhsic_config *uhsic_config = phy->config;

	if (phy->usb_phy_type == TEGRA_USB_PHY_TYPE_HSIC) {
		printk("tegra_usb_phy_bus_connect is called\n");
#ifndef CONFIG_ARCH_TEGRA_2x_SOC
		val = readl(base + TEGRA_USB_USBMODE_REG_OFFSET);
		val |= TEGRA_USB_USBMODE_HOST;
		writel(val, base + TEGRA_USB_USBMODE_REG_OFFSET);
		...
		if(b_first == 0)
		{
			if(!gpio_request(ulpi_config->bb_data->modem.xmm.bb_rst, "bb_reset") && 
					!gpio_request(ulpi_config->bb_data->modem.xmm.bb_on, "bb_pwron"))
			{
				gpio_direction_output(ulpi_config->bb_data->modem.xmm.bb_rst, 0);
				gpio_direction_output(ulpi_config->bb_data->modem.xmm.bb_on, 0);
				tegra_gpio_enable(ulpi_config->bb_data->modem.xmm.bb_rst);
				tegra_gpio_enable(ulpi_config->bb_data->modem.xmm.bb_on);

				mdelay(200);
				gpio_set_value(ulpi_config->bb_data->modem.xmm.bb_rst, 1);
				mdelay(300);
				gpio_set_value(ulpi_config->bb_data->modem.xmm.bb_on, 1);

				gpio_free(ulpi_config->bb_data->modem.xmm.bb_rst);
				gpio_free(ulpi_config->bb_data->modem.xmm.bb_on);

				b_first=1;
				printk("%s power sequence done\n", __func__);
				
			}
			else
				pr_err("%s: BB_rst or/ BB_pwron gpio_request failed!\n", __func__);
		}
#endif		
		...
		printk("[tegra_usb_phy_bus_connect] UHSIC_CONNECT_DETECT detected!!\n");
...
int tegra_usb_phy_bus_reset(struct tegra_usb_phy *phy)
{
	...
	if (phy->usb_phy_type == TEGRA_USB_PHY_TYPE_HSIC) {
		printk("tegra_usb_phy_bus_reset is called\n");
#ifndef CONFIG_ARCH_TEGRA_2x_SOC
		/* Change the USB controller PHY type to HSIC */
		val = readl(base + HOSTPC1_DEVLC);
		val &= ~HOSTPC1_DEVLC_PTS(HOSTPC1_DEVLC_PTS_MASK);
...
int tegra_usb_phy_bus_idle(struct tegra_usb_phy *phy)
{
	...
	if (phy->usb_phy_type == TEGRA_USB_PHY_TYPE_HSIC) {
		printk("tegra_usb_phy_bus_idle is called\n");
#ifndef CONFIG_ARCH_TEGRA_2x_SOC
		val = readl(base + TEGRA_USB_USBMODE_REG_OFFSET);
		val |= TEGRA_USB_USBMODE_HOST;
		writel(val, base + TEGRA_USB_USBMODE_REG_OFFSET);
