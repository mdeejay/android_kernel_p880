...
int tegra_update_cpu_speed(unsigned long rate)
{
	 ...
	 */
	if (freqs.old < freqs.new) {
		ret = tegra_cpu_scale_sclk_rate(freqs.new);
		if (ret) {
			pr_err("cpu-tegra: Failed to scale sclk for cpu"
			       " frequency %u kHz\n", freqs.new);
			return ret;
		}
		ret = tegra_update_mselect_rate(freqs.new);
		if (ret) {
			printk("cpu-tegra: Failed to scale mselect for cpu"
			       " frequency %u kHz ret = %d\n", freqs.new,ret);
			return ret;
		}
		ret = clk_set_rate(emc_clk, tegra_emc_to_cpu_ratio(freqs.new));
		if (ret) {
			printk("cpu-tegra: Failed to scale emc for cpu"
			       " frequency %u kHz ret = %d\n", freqs.new,ret);
			return ret;
		}
	}
	...
	if (ret) {
		printk("cpu-tegra: Failed to set cpu frequency to %d kHz, ret = %d\n",
			freqs.new,ret);
		return ret;
	}
	...
	if (freqs.old > freqs.new) {
		...
		tegra_cpu_scale_sclk_rate(freqs.new);
	}
