		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	//printk("led_brightness_store : android brightness to %d \n", state);
	if (isspace(*after))
		count++;

	if (count == size) {
		ret = count;
		//printk("led_brightness_store : input brightness to %d \n", state);
		if ((state < LED_OFF) || (state > LED_FULL )) /*Invalid brightness*/
			rc = -EINVAL;
		else {
			if (state == LED_OFF){
			...
			//printk("led_brightness_store : set brightness to %d \n", state);
		}
	}
