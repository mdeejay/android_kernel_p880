			...
			if (likely(t->use_rx_dma)) {
				//if (!is_rx_int) {
					is_rx_int = true;
                                        ...
                                        ier |= UART_IER_RDI;
                                        uart_writeb(t, ier, UART_IER);
                                        uart_readb(t, UART_IER); /* make sure writing is finished */
                                        ier &= ~(UART_IER_RDI | UART_IER_RLSI | UART_IER_RTOIE | UART_IER_EORD);
                                        t->ier_shadow = ier;
                                        uart_writeb(t, ier, UART_IER);
                                        udelay(5);
                                        uart_readb(t, UART_IER); /* make sure writing is finished */
                                        if (cnt_clear_eord > 10)
						printk("%s count of clearing eord = %d\n", __func__, cnt_clear_eord);
                                        cnt_clear_eord++;
                                //}
                        } else {
				do_handle_rx_pio(t);
