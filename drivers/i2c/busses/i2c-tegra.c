...
// Jake Park : testing for bug 937691 on X3
	if (status == 0) {
		u32 _cnfg, _status, _transfer_status, _fifo_status;
		u32 _int_mask, _int_status, _int_source;

		_cnfg		= i2c_readl(i2c_dev, I2C_CNFG);
		_status		= i2c_readl(i2c_dev, I2C_STATUS);
		_transfer_status= i2c_readl(i2c_dev, I2C_PACKET_TRANSFER_STATUS);
		_fifo_status	= i2c_readl(i2c_dev, I2C_FIFO_STATUS);
		_int_mask	= i2c_readl(i2c_dev, I2C_INT_MASK);
		_int_status	= i2c_readl(i2c_dev, I2C_INT_STATUS);
		_int_source	= i2c_readl(i2c_dev, 0x70 /*I2C_INT_SOURCE*/);

		printk("OLD ==> %8x %8x %8x %8x %8x %8x %8x\n",
		       _cnfg, _status, _transfer_status,
		       _fifo_status, _int_mask, _int_status, _int_source);
		...
		printk("NEW ==> %8x %8x %8x %8x %8x %8x %8x\n",
		       _cnfg, _status, _transfer_status,
		       _fifo_status, _int_mask, _int_status, _int_source);

		status = i2c_readl(i2c_dev, I2C_INT_STATUS);
	}
// Jake Park : testing bug 937691
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	...
	/* Because work can be run in [suspend/resume]_noirq state, don't disable IRQ on suspend.
	 * Let the HW control Enabling/disabling interrupt */
