...
static ssize_t k3g_show_gyrodata(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	...
	res = k3g_read_gyro_values(k3g_data->client, &data,
				k3g_data->entries + k3g_data->drop_next_event);
	if (res < 0)
		return sprintf(buf, "Can't Read Gyro Data\n", strbuf);

	memset(strbuf, 0x00, 20);
	sprintf(strbuf, "%d, %d, %d", data.x, data.y, data.z);
...
static long k3g_misc_ioctl_handler(struct file *file, unsigned int cmd,
					 unsigned long arg, void __user *p)
{
    void __user *argp = (void __user *)arg;

    if(DEBUG_FUNC_TRACE & debug_mask)
	printk(KERN_INFO "%s: %s call with cmd(K3G_IOCTL_READ_DATA_XYZ= 0x%x) 0x%x and arg 0x%x\n",
		DRV_NAME, __func__, K3G_IOCTL_READ_DATA_XYZ, cmd, (unsigned int)arg);
...
{
	return k3g_misc_ioctl_handler(file, cmd, arg, (void __user *)arg);
}


#ifdef CONFIG_COMPAT
static long k3g_misc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return k3g_misc_ioctl_handler(file, cmd, arg, compat_ptr(arg));
}
#endif
...
#if CONFIG_COMPAT
		.compat_ioctl = k3g_misc_compat_ioctl,
#endif
