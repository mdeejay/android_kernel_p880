...
static long k3dh_acc_misc_ioctl_handler(struct file *file, unsigned int cmd,
...
{
	return k3dh_acc_misc_ioctl_handler(file, cmd, arg, (void __user *)arg);
}


#ifdef CONFIG_COMPAT
static long k3dh_acc_misc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return k3dh_acc_misc_ioctl_handler(file, cmd, arg, compat_ptr(arg));
}
#endif
...
#if CONFIG_COMPAT
		.compat_ioctl = k3dh_acc_misc_compat_ioctl,
#endif
...
static ssize_t k3dh_store_tap_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
 	unsigned long val = simple_strtoul(buf, NULL, 10);
  	...
  	else
  	{
    	...
    	k3dh_regs_update(k3dh_acc_misc_data,TT_THS,RES_TT_THS,(0x50|0x80));
...
static ssize_t k3dh_xyzdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d, %d, %d\n", k3dh_xyz[0],k3dh_xyz[1],k3dh_xyz[2]);
}
...
static DEVICE_ATTR(xyzdata, S_IRUGO, k3dh_xyzdata_show, NULL);		
...
static int k3dh_acc_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	u8 id_check = WHO_AM_I;
  	...
  	{
    	dev_err(&client->dev, "client not smb-i2c capable - I2C_FUNC_SMBUS_BYTE:2\n");
    	err = -EIO;
    	goto exit_check_functionality_failed;
  	}
	...
	INIT_WORK(&acc->hr_work, k3dh_acc_input_work_func);
  	...
  	/* read chip id */
	err = k3dh_acc_i2c_read(acc, &id_check, 1);
	if(id_check != WHOAMI_K3DH_ACC)
	{
    	acc->client = NULL;
    	printk(KERN_INFO "%s I2C driver not registered!"
    	" Device unknown\n", K3DH_ACC_DEV_NAME);
	    goto err_destoyworkqueue2;
