...
static long ami306_ioctl_handler(struct file *file, unsigned int cmd,
...
{
	return ami306_ioctl_handler(file, cmd, arg, (void __user *)arg);
}


#ifdef CONFIG_COMPAT
static long ami306_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return ami306_ioctl_handler(file, cmd, arg, compat_ptr(arg));
}
#endif
...
static long ami306daemon_ioctl_handler(struct file *file, unsigned int cmd,
				 unsigned long arg, void __user *p)
{
	int valuebuf[4];
#if defined(STM_GYRO)	
...
{
	return ami306daemon_ioctl_handler(file, cmd, arg, (void __user *)arg);
}


#ifdef CONFIG_COMPAT
static long ami306daemon_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return ami306daemon_ioctl_handler(file, cmd, arg, compat_ptr(arg));
}
#endif
...
static int ami306hal_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
...
#if CONFIG_COMPAT
	.compat_ioctl = ami306_compat_ioctl,
#endif
...
#if CONFIG_COMPAT
	.compat_ioctl = ami306daemon_compat_ioctl,
#endif	
};
		...
		const struct i2c_device_id * devid)
{
	...
	ami306mid_data.controldata[AMI306_CB_MDIR] = pdata->fdata_mDir;  
