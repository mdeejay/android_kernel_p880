...
struct nvavp_info {
	...
	int				power_mgt;
	struct mutex			power_mgt_lock;
	struct delayed_work		power_mgt_work;
...
#define POWER_MGT_WORK_DELAY				10000 /* msec */
#define POWER_MGT_JPEG_DECODE_CPU_USER_CAP_RATE		1000000 /* KHz */
#define POWER_MGT_JPEG_DECODE_MAX_CPUS			2

static void tegra_nvavp_do_power_mgt_work(struct work_struct *work)
{
	struct nvavp_info *nvavp =
		container_of(work, struct nvavp_info, power_mgt_work.work);

	if (!nvavp->power_mgt)
		return;

	mutex_lock(&nvavp->power_mgt_lock);

	if (nvavp->jpeg_decode) {
		tegra_cpu_user_cap_set(0);
	...
	mutex_unlock(&nvavp->power_mgt_lock);
}

static inline void tegra_nvavp_do_power_mgt(struct nvavp_info *nvavp)
{
	if (!nvavp->power_mgt && nvavp->jpeg_decode)
		nvavp->power_mgt = 1;
	...
	if (nvavp->jpeg_decode) {
		tegra_cpu_user_cap_set(POWER_MGT_JPEG_DECODE_CPU_USER_CAP_RATE);
		tegra_auto_hotplug_set_max_cpus(POWER_MGT_JPEG_DECODE_MAX_CPUS);
		schedule_delayed_work(&nvavp->power_mgt_work,
				      msecs_to_jiffies(POWER_MGT_WORK_DELAY));
	} else {
		tegra_cpu_user_cap_set(0);
		tegra_auto_hotplug_set_max_cpus(0);
		cancel_delayed_work_sync(&nvavp->power_mgt_work);
		nvavp->power_mgt = 0;
	}
}

static ssize_t tegra_nvavp_show_jpeg_decode(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct nvavp_info *nvavp = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", nvavp->jpeg_decode);
}

static ssize_t tegra_nvavp_store_jpeg_decode(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == nvavp->jpeg_decode)
		return count;

	mutex_lock(&nvavp->power_mgt_lock);
	nvavp->jpeg_decode = val;
	tegra_nvavp_do_power_mgt(nvavp);
	mutex_unlock(&nvavp->power_mgt_lock);

	return count;
}

static DEVICE_ATTR(jpeg_decode, S_IRUGO | S_IWUSR | S_IRGRP | S_IWGRP,
		   tegra_nvavp_show_jpeg_decode, tegra_nvavp_store_jpeg_decode);
...
static int tegra_nvavp_probe(struct nvhost_device *ndev)
{
	...
	mutex_init(&nvavp->power_mgt_lock);
	INIT_DELAYED_WORK_DEFERRABLE(&nvavp->power_mgt_work,
				     tegra_nvavp_do_power_mgt_work);

	ret = device_create_file(&ndev->dev, &dev_attr_jpeg_decode);
	if (ret < 0)
		dev_warn(&ndev->dev,
			 "%s: failed to add jpeg_decode sysfs: %d\n",
			 __func__, ret);
...
static int tegra_nvavp_remove(struct nvhost_device *ndev)
{
	...
	device_remove_file(&ndev->dev, &dev_attr_jpeg_decode);
	cancel_delayed_work_sync(&nvavp->power_mgt_work);
	mutex_destroy(&nvavp->power_mgt_lock);
