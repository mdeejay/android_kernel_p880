...
int PM_control = TRUE;
#if 0
extern int dhdcdc_query_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len, uint8 action);	// LGE_patch
#endif
...
// LGE_CHANGE_S, jooin.woo, 20120209, for OTA test to fix PM
#if defined(CONFIG_LGE_BCM432X_PATCH)
				if(PM_control){
#endif
// LGE_CHANGE_E, jooin.woo, 20120209, for OTA test to fix PM
...
/* 20120508 sangjun.bae@lge.com no need to check default mac because COB has already it's own default mac[START]*/
/*
...
*/
/* 20120508 sangjun.bae@lge.com no need to check default mac because COB has already it's own default mac[END]*/
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	// Temp -  Ignore I/O control
	bool temp_NV_MAC_valid = FALSE;
...
/* LGE_CHANGE_E [jooin.woo@lge.com] 2012-12-12, mac write */
	...
	//	}
		...
		if (!ret) {
			// Temp -  Ignore I/O control
			temp_NV_MAC_valid = dhd_check_nvmac_is_valid(nvbuf_ether_addr);

			// Temp -  Ignore I/O control
			//if (dhd_check_nvmac_is_valid(nvbuf_ether_addr) == TRUE) {			
			if (temp_NV_MAC_valid == TRUE) {
...
//void
int // /* LGE_PATCH */ //workaround ioctl error
