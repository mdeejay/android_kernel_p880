...
void SiiMhlTxRcpeSend(u8 erro_code, u8 key_code)
{
  ...
  cbus_cmd_bind(MHL_MSC_MSG,0x00,data,0x03, FALSE ); 
}
...
void simg_timer_schdule(struct work_struct *work)
{
	...
	if((sys_stat & RSEN_HIGH) == 0)
	{
		...
		
		{
			...
			ForceUsbIdSwitchOpen();
			simg_msleep(50);
			printk("[MHL] simg_timer_schdule: ForceUsbIdSwitchOpen\n");
			ReleaseUsbIdSwitchOpen();
			delete_all_cbus_cmd(); 
...
int process_mhl_discovery_start(void)
{
    ...
    if((int4Status & MHL_EST_INTR4)&&(mhl_status_value.cbus_connected))
    {
		...
		if(rsen_timer == FALSE){
			rsen_timer = TRUE;
			printk("[MHL] process_mhl_discovery_start: timer start\n");
...
#endif
    printk("[MHL] CbusWakeUpPulseGenerator: Discovery start\n");
    return;
}
...
static void InitCBusRegs (void)
{
	...
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x89, 0xf); // BANDWIDTH 15
	
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8A, (MHL_FEATURE_RCP_SUPPORT | MHL_FEATURE_RAP_SUPPORT |MHL_FEATURE_SP_SUPPORT));

	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8B, 0x0); // DEVICE_ID_H 0
	I2C_WriteByte(PAGE_CBUS_0XC8, 0x8C, 0x0); // DEVICE_ID_L 0
