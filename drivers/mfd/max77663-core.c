...
static int max77663_sleep_enable(struct max77663_chip *chip, int on)
{
	int ret = 0;
...
static int max77663_set_32kHz_Oscillator(struct max77663_chip *chip)
{
	int ret;

	ret = max77663_set_bits(chip->dev, MAX77663_REG_32KHZ_OSC,
					OSC_32KHZ_LOAD_MASK, OSC_32KHZ_LOAD_12PF, 0);
	if (ret < 0)
		dev_err(chip->dev,
			"sleep_enable: Failed to set manual reset time config1\n");
...
#define MAX77663_RTC_UPDATE0		0x04
	...
	ret = max77663_write(chip->dev, MAX77663_RTC_UPDATE0, &val, 1, 1);
	if (ret < 0)
		return ret;
			  ...
			  const struct i2c_device_id *id)
{
	...
	max77663_sleep_enable(chip, false);
...
static int max77663_suspend(struct device *dev)
{
	...
	ret = max77663_sleep_enable(chip, true);
	if (ret < 0) {
...
#if defined(CONFIG_MACH_X3_DUMP_GPIO)
...
static int max77663_resume(struct device *dev)
{
	...
	ret = max77663_sleep_enable(chip, false);
	if (ret < 0) {
